...
const flatDetails = numberToFlats(flatNumber = +prompt("Введіть номер квартири"), flatsPerFloor = +prompt("Введіть кількість квартир на поверсі"), floorsPerEntrance = +prompt("Введіть кількість поверхів в під'їзді"));
...
function allName(){
    let name, surname, fatherName, fullName;
    name = prompt("Введіть ваше ім'я:") || "Іван" ;
    surname = prompt("Введіть ваше прізвище:") || "Іванов";
    fatherName = prompt("Введіть ваше по батькові:") || "Іванович";
    fullName = capitalize(name) + " " + capitalize(surname) + " " + capitalize(fatherName);
...
const promptOrDefault = (message, defaultValue) => prompt(message) || defaultValue ;
let input1 = promptOrDefault("Введіть текст:", "Значення за замовчуванням")
console.log(input1);
...
// Filter Lexics
    ...
    function filterLexics(inputString, forbiddenWords) {
        ...
        console.log(words);
        const filteredWords = words.filter(word => forbiddenWords.indexOf(word) === -1);
    return filteredWords.join(" ");
    }
    const input = prompt("Введіть речення для перевірки") || "Це речення містить слова бляха, муха, пляшка та шабля";
