// Confirms
{
    const arr = [confirm('do you like dog?'),
...
// IndexOf Word
{
    ...
    (result == false)? alert("такого слова не має в реченні") : alert(` ваше слово має номер = ${result+2}`);
}
// Reverse
{
    let array = [];
...
}// Flat
{
    ...
    let arr2 = arr.reduce((accumulator, currentValue) => [...accumulator, ...currentValue], []);
    console.log(arr);
    console.log(arr2);
...
// Multiply table rest
{
    ...
    const [[, ...firstRest], [, , ...secondRest], [, , , ...thirdRest], [, , , , ...fourthRest]] = arr;
    const combinedArray = [...firstRest, ...secondRest, ...thirdRest, ...fourthRest];
...
// Beep Lexics
{
    ...
    let result = words.map(word => bannedWords.includes(word) ? "BEEP" : word);
...
// For Brackets Hell Check
{
    ...
    for (const character of line) {
        if (character === '[' || character === '(' || character === '{') {
            ...
            if (!lastBracket || (character === ']' && lastBracket.character !== '[') || (character === ')' && lastBracket.character !== '(') || (character === '}' && lastBracket.character !== '{')) {
                errorIndex = i;
                break;
            }
        }
